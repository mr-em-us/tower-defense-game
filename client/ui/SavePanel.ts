...
export class SavePanel {
  ...
  private async fetchAndRender(): Promise<void> {
    try {
      const resp = await fetch(`${window.location.origin}/api/saves?player=${encodeURIComponent(this.playerName)}`);
  ...
  private renderList(saves: SaveMetadata[]): void {
    ...
    for (const save of saves) {
      ...
      delBtn.addEventListener('click', async () => {
        await fetch(`${window.location.origin}/api/saves?id=${save.id}&player=${encodeURIComponent(this.playerName)}`, { method: 'DELETE' });
        await this.fetchAndRender();
      });
