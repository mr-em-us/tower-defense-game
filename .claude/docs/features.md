...
Server-side AI opponent for singleplayer. Virtual client pattern: AIController inside GameRoom produces ClientMessages through same validation pipeline as human players. Three difficulties (Easy/Medium/Hard) via depth dial (0.25/0.55/0.90) — same resources, same information, deeper evaluation. **Strategy: Emergent maze builder** — greedy hill-climbing on path length. Each tower placement scored by `pathDelta×15 + pathCoverage×2 + wallAdjacency×3 + pathProximity×1`. Lexicographic sort (delta>0 first). No predefined geometry — maze emerges from simple scoring rules. Path ~86 cells on wave 1. Median wave 16 at speed=4 (13 tests). AA defense with proactive targeting. Files: server/ai/ (AIController.ts, names.ts, strategies/*.ts), shared/types/ (AIDifficulty enum, aiEnabled/aiDifficulty settings, isAI player flag).
